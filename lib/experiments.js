 ...
 * @returns {PromiseLike<Experiment>}
 */
function saveExperiment (redis, { namespace, name, weight, disabled, flags }) {
  const key = `exp:${namespace}:${name}`
  const flagArgs = R.pipe(
    R.map(v => v ? '1' : '0'),
    R.toPairs,
    R.flatten)(flags)
  const disabledValues = disabled ? [ '@disabled', 'true' ] : []
  return redis.hmset(key, ...['@weight', weight.toString(), ...disabledValues, ...flagArgs])
    .then(() => !disabled && redis.hdel(key, '@disabled'))
    .then(() => ({
      namespace,
      name,
      disabled,
...
const parseExperiment = key => fields => ({
  ...
  disabled: Object.keys(fields).includes('@disabled'),
