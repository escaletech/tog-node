...
describe('get experiment', () => {
  ...
  test('disabled experiment', () => {
    const redis = {
      hgetall: jest.fn().mockResolvedValue({ '@disabled': 'true' })
    }
...
describe('list experiments', () => {
  test('returning some experiments', () => {
    const storage = {
      ...
      'exp:foo:three': { '@weight': '50', '@disabled': 'true' } // disabled
    }
    const redis = {
    ...
    return listExperiments(redis, 'foo')
      .then(result => {
        expect(result).toMatchObject([
          { name: 'one', weight: 50, disabled: false, flags: {} },
          { name: 'two', weight: 50, disabled: false, flags: { red: true, rounded: false } },
          { name: 'three', weight: 50, disabled: true, flags: {} }
        ])
...
describe('create experiment', () => {
  test('sets experiment without flags', () => {
    const redis = { hmset: jest.fn().mockResolvedValue() }

    return createExperiment(redis, 'foo', 'bigger-cta', 50, {})
      .then(() => {
  ...
  test('sets experiment with flags', () => {
    ...
    return createExperiment(redis, 'foo', 'bigger-cta', 50, { red: true, rounded: false })
      .then(() => {
        expect(redis.hmset)
